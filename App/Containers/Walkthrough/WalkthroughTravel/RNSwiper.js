...
import { View, PanResponder, Dimensions, Easing, Animated, StyleSheet, Text,Platform } from 'react-native';
		...
		return(
			<Animated.View style={[styles.container, {marginLeft: this.state.containerOffset}]} {...this._panResponder.panHandlers}>
              ...
              <View style={{zIndex: this.state.zIndex[0],}} key={i}>
  							<Animated.View key={i} style={[{...Platform.select({ios: {marginRight: this.overlap * -1},android: {marginRight: this.overlap * -0.5}}),zIndex: this.state.zIndex[0],width:this.props.cardWidth, opacity: this.state.opacityToFade,transform: [{scale: this.state.scaleToShrink}]}]} >
  								{card}
