              ...
              <View style={{zIndex: this.state.zIndex[0],}} key={i}>
  							<Animated.View key={i} style={[{marginRight: this.overlap * 0.5,zIndex: this.state.zIndex[0],width:this.props.cardWidth, opacity: this.state.opacityToFade,transform: [{scale: this.state.scaleToShrink}]}]} >
									<View style={styles.contentStyle}>
										{
											swiperDetails.map((swiperDetails, k) => {
												return(
													<View key={k}>
														{
															(i == k) ?
						...
						return (
							<Animated.View key={i} style={{marginTop:(Dimensions.get('window').height * 0.21), marginRight: this.overlap * -1,marginLeft: this.overlap * -1,zIndex: this.state.zIndex[1],width:this.props.cardWidth, opacity: this.state.opacityToAppearRight, transform: [{scale: this.state.scaleToGrowRight}]}} >
								{card}
							</Animated.View>
						)
					} else if (i === this.state.activeX - 1){
						return (
							<Animated.View key={i} style={{marginTop:(Dimensions.get('window').height * 0.21), marginRight: this.overlap * -1,marginLeft: this.overlap * 0,zIndex: this.state.zIndex[2],width:this.props.cardWidth, opacity: this.state.opacityToAppearLeft, transform: [{scale: this.state.scaleToGrowLeft}]}} >
								{card}
							</Animated.View>
						)
					} else {
						return (
							<Animated.View key={i} style={{marginTop:(Dimensions.get('window').height * 0.21), marginRight: this.overlap * -1,marginLeft: this.overlap * 0,zIndex: this.state.zIndex[3],width:this.props.cardWidth, opacity:this.minimumOpacity, transform: [{scale: this.minimumScale}]}} >
								{card}
							</Animated.View>
						)
					}
